        ...
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700">
              ...
              <th class="px-5 py-3 text-left">Approval Steps</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="loan in loans" :key="loan.id">
              ...
              <td class="px-5 py-4 text-right">৳{{ Number(loan.principal_amount).toLocaleString() }}</td>
              <td class="px-5 py-4 text-right">৳{{ Number(loan.emi_amount).toLocaleString() }}</td>
              <td class="px-5 py-4 text-right font-semibold"
                  :class="loan.remaining_balance == 0 ? 'text-green-600' : 'text-red-600'">
                ৳{{ Number(loan.remaining_balance).toLocaleString() }}
              </td>
              <td class="px-5 py-4">
                <span class="px-2 py-1 rounded text-xs" :class="statusClass(loan.status)">
                  {{ loan.status }}
                </span>
              </td>
              ...
              <td class="px-5 py-4">
                <div class="flex flex-col gap-1">
                  <span
                    v-for="step in loan.approval?.steps || []"
                    :key="step.id"
                    class="px-2 py-1 rounded text-xs"
                    :class="approvalStepClass(step.status)"
                  >
                    {{ step.role.name }}: {{ step.status }}
                  </span>
                </div>
              </td>
              ...
              <td class="px-5 py-4 text-right">
                <div class="flex justify-end gap-3">
                  <router-link :to="`/admin/loans/${loan.id}`" class="text-indigo-600 hover:underline">View</router-link>
                  <router-link :to="`/admin/loans/${loan.id}/edit`" class="text-blue-600 hover:underline">Edit</router-link>
                  <button @click="deleteLoan(loan.id)" class="text-red-600 hover:underline">Delete</button>
                </div>
              </td>
            </tr>

            <tr v-if="loans.length === 0">
              <td colspan="8" class="text-center py-6 text-gray-500">No loans found.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const fetchLoans = async () => {
  try {
    ...
    // Make sure to eager load approval steps in your API
